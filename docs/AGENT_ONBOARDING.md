...
- The initial iteration image prompt is generated by the artist LLM from fixed vision + soul context (not hardcoded templates).
...
- Reject low-information critique/action outputs (for example fragmentary feedback or one-word actions) and retry once with stricter prompting.
